(() => {
  ...
  const setError = (field, message) => {
    ...
    const error = field
      .closest(".auth-field")
      ?.querySelector(".auth-field-error");
  ...
  const shakeInvalid = () => {
    ...
    if (invalid.length)
      gsap.fromTo(
        invalid,
        { x: -5 },
        { x: 0, duration: 0.32, ease: "power2.out", stagger: 0.025 },
      );
  };
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (mode === "signup") {
      ...
      window.setTimeout(() => {
        window.location.replace(
          `login.html?email=${encodeURIComponent(emailValue)}`,
        );
      }, 300);
      return;
    }
    ...
    localStorage.setItem(
      "stacklyUserName",
      localStorage.getItem("stacklyUserName") || emailValue.split("@")[0],
    );
    localStorage.setItem("stacklyRole", role.value);
    localStorage.setItem(
      "stacklyAuthSession",
      JSON.stringify({ email: emailValue, role: role.value }),
    );
  ...
  if (page.dataset.authMode === "login") {
    ...
    if (emailInput && !emailInput.value)
      emailInput.value = queryEmail || savedEmail || "";
  }
})();
