(()=>{
 ...
 if(window.AOS) AOS.init({duration:620,easing:'ease-out-cubic',once:true,offset:30,disable:reduce});
 ...
 document.querySelector('[data-sidebar-close]')?.addEventListener('click',()=>{
   if(window.innerWidth<=820){closeSidebar();return;}
   const collapsed=sidebar?.classList.toggle('is-collapsed');
   app.classList.toggle('sidebar-collapsed',collapsed);
   localStorage.setItem('stacklySidebarCollapsed',collapsed?'1':'0');
 });
 if(window.innerWidth>820&&localStorage.getItem('stacklySidebarCollapsed')==='1'){sidebar?.classList.add('is-collapsed');app.classList.add('sidebar-collapsed')}
 ...
 document.querySelector('[data-profile-toggle]')?.addEventListener('click',()=>profile?.classList.toggle('is-open'));
 document.addEventListener('click',e=>{if(profile&&!profile.contains(e.target))profile.classList.remove('is-open')});
 ...
 document.querySelector('[data-search]')?.addEventListener('input',e=>{
   ...
   document.querySelectorAll('[data-searchable]').forEach(item=>{item.hidden=!!q&&!item.textContent.toLowerCase().includes(q)});
 });
