...
document.addEventListener("DOMContentLoaded", () => {
  if (window.AOS) {
    AOS.init({duration:800, easing:"ease-out-cubic", once:true, offset:70, disable:() => window.matchMedia("(prefers-reduced-motion: reduce)").matches});
  }

  const newsletter = document.querySelector(".newsletter");
  const email = document.querySelector("#email");
  const message = document.querySelector(".newsletter-message");
  if (newsletter && email && message) {
    newsletter.addEventListener("submit", (event) => {
      event.preventDefault();
      const value = email.value.trim();
      const valid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);
      email.setAttribute("aria-invalid", String(!valid));
      message.textContent = valid ? "You’re on the list — welcome to better care." : "Please enter a valid email address.";
  ...
  if (window.gsap && window.ScrollTrigger) {
    ...
    gsap.from(".hero h1", {y:35, opacity:0, duration:1.05, ease:"power3.out", delay:.1});
    gsap.to(".orbit-a", {rotation:360, duration:34, repeat:-1, ease:"none"});
    ...
    gsap.utils.toArray(".care-card-art").forEach((art, i) => {
      ...
      if (core) gsap.to(core, {y:-6, duration:2.8+i*.15, repeat:-1, yoyo:true, ease:"sine.inOut", delay:i*.12});
    });
    gsap.utils.toArray(".care-ring").forEach((ring, i) => {
      gsap.to(ring, {scale:1.035, opacity:.72, duration:2.8+i*.15, repeat:-1, yoyo:true, ease:"sine.inOut", delay:i*.1});
    });
    gsap.to(".route-line:after", {x:260, duration:2.6, repeat:-1, ease:"none"});
    gsap.utils.toArray(".route-node").forEach((node, i) => {
      const icon = node.querySelector("i");
      if (icon) gsap.to(icon, {y:-4, duration:2.2+i*.12, repeat:-1, yoyo:true, ease:"sine.inOut", delay:i*.12});
    });
    gsap.utils.toArray(".memory-people span").forEach((el,i) => {
      gsap.to(el, {y:-8, duration:1.7+i*.15, repeat:-1, yoyo:true, ease:"sine.inOut", delay:i*.12});
    });
    ...
    if (hw05Cards.length) {
      ...
      const revealHw05 = () => {
        ...
        gsap.fromTo(hw05Cards,
          { opacity: 0, y: 28, scale: .985 },
          { opacity: 1, y: 0, scale: 1, duration: .72, stagger: .1, ease: "power3.out", clearProps: "transform" }
        );
      };
      ...
      window.setTimeout(() => {
        if (!revealHw05.done) {
          revealHw05.done = true;
          gsap.set(hw05Cards, { opacity: 1, y: 0, scale: 1, clearProps: "transform" });
        }
      }, 1400);

      const activateStep = (card, animate = true) => {
        const step = Number(card.dataset.step || 1);
        hw05Cards.forEach((item) => item.classList.toggle("is-active", item === card));
        hw05Dots.forEach((dot, index) => dot.classList.toggle("is-active", index + 1 === step));
        ...
        if (animate && window.gsap && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
          ...
          if (icon) gsap.fromTo(icon, {scale:.92, rotate:-4}, {scale:1, rotate:0, duration:.5, ease:"back.out(2)"});
          if (tags.length) gsap.fromTo(tags, {y:7, opacity:.45}, {y:0, opacity:1, duration:.35, stagger:.05, ease:"power2.out"});
        }
      };
