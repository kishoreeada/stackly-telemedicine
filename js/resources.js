(() => {
  ...
  const resourceForm = document.querySelector('.resource-newsletter');
  const resourceEmail = document.querySelector('#resource-email');
  const resourceMessage = document.querySelector('.resource-newsletter-message');
  resourceForm?.addEventListener('submit', event => {
    event.preventDefault();
    const valid = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i.test(resourceEmail?.value.trim() || '');
    if (!valid) {
      resourceEmail?.setAttribute('aria-invalid', 'true');
      if (resourceMessage) resourceMessage.textContent = 'Please enter a valid email address.';
      return;
    }
    resourceEmail?.removeAttribute('aria-invalid');
    if (resourceMessage) resourceMessage.textContent = 'Thank you — you’re connected.';
    resourceForm.reset();
  });

  resourceEmail?.addEventListener('input', () => {
    resourceEmail.removeAttribute('aria-invalid');
